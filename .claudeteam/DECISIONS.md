...
- **Tarih:** 2026-05-09
- **Durum:** Kabul edildi (Sprint #4 T-#4-001)
- **Karar verenler:** ML/RAG Engineer, Tech Lead, Senior Dev #1, AppSec, PO (Aykut)

**Bağlam:**
Mevcut chat flow'unda KB'nin tamamı (50 entry, ~13K token) her sorguda system prompt'a yığılıyor — `ARCHITECTURE.md`'de "RAG değil, retrieval lite" tech debt olarak işaretliydi. Hem token maliyeti, hem KB 200+ entry'e büyüyemez (context window patlar), hem model gürültü içinde kayboluyor (relevance düşük). Brief: `RESEARCH_BRIEFS/rag-migration.md` (Sprint #2'de yazıldı, 6 fazlı plan).

**Değerlendirilen Seçenekler:**
1. **PGvector + Gemini text-embedding-004** (seçildi) — mevcut Postgres'te ek kolon, vendor lock-in yok, Gemini ekosisteminde kal (ek API key yok), ücretsiz extension
2. Pinecone / Qdrant / Weaviate — managed vector DB, fazladan SaaS bağımlılığı, KB 50 entry için over-engineered, ücret
3. OpenAI text-embedding-3-small — daha olgun ama ek API key, ek vendor, Gemini ekosisteminden çıkış
4. SQLite + sqlite-vss — dev'de hızlı ama prod Postgres ile uyumsuz
5. Status quo (KB stuffing) — token israfı, scale impossible

**Karar:**
PGvector extension + Pgvector.EntityFrameworkCore (0.3.0) + Gemini text-embedding-004 (768 dim) + HNSW index (`vector_cosine_ops`). Implementation 6 faza bölünüp tek seferde merge edildi:
- **Faz 1:** Docker image `pgvector/pgvector:pg{15,16}`, EF migration `AddKnowledgeEmbedding` (CREATE EXTENSION + vector(768) kolonu + HNSW raw SQL)
- **Faz 2:** `IEmbeddingService` + `GeminiEmbeddingService` (native :embedContent endpoint, exponential backoff retry, transient classification)
- **Faz 3:** KB Create/Update embed compute, `SearchSimilarAsync(query, topK)`, `BackfillMissingEmbeddingsAsync()`
- **Faz 4:** AiService chat path `UseRagRetrieval=true` (default) → `SearchSimilarAsync(userMessage, RetrievalTopK=5)`, fallback to KB-stuffing on retrieval miss
- **Faz 5:** `.claudeteam/EVAL_SET/rag-eval.json` (15 Q&A keyword-check) + `--eval-rag` CLI
- **Faz 6:** ADR-012 (bu doc), brief status update

**Rationale:**
- **Postgres native:** Mevcut DB'de yaşar, ek altyapı yok, backup pipeline'ı aynı, deploy aynı
- **Gemini key share:** `AiSettings.ApiKey` (GEMINI_API_KEY) embedding'de de fallback olarak kullanılır → tek secret yönetimi
- **Graceful fallback:** Embedding service down / API key yoksa eski KB stuffing path'ine düşer → chat asla bozulmaz
- **Feature flag:** `AiSettings.UseRagRetrieval` runtime'da toggle; sorun çıkarsa `false` yapıp config reload
- **Backfill safety:** `BackfillMissingEmbeddingsAsync` idempotent, sadece NULL embedding'leri günceller, partial success raporlar
- **Eval as guard:** Keyword-check coarse ama "RAG hiç entry getiremedi" / "model konu dışına çıktı" gibi büyük regression'ları yakalar

**Sonuçlar / Trade-off'lar:**
- ✅ Token maliyet: ~13K → ~1.5K input/chat (≈%88 düşüş, ölçek arttıkça önemli)
- ✅ Scale: KB 10K+ entry'e kadar HNSW O(log N)
- ✅ Relevance: model "yığın"da kaybolmuyor, top-5 odaklı context
- ✅ Eval altyapısı: yan ürün, T-#4-003 Inbox + gelecek AI işlerine baz
- ⚠️ Embedding API maliyeti: KB Create/Update'te 1 call (ucuz, cache'lenmez — text değişti = recompute)
- ⚠️ Backfill ilk kez çalışırken 50 sequential call (~10sn) — 1500 RPM limit altında rahat
- ⚠️ Gemini embedding rate limit: chat ile aynı API key paylaşır, prod'da concurrent yüksekse ayrı key düşünülür
- ⚠️ HNSW default params (m=16, ef=64) küçük KB için optimize değil — 200+ entry'de tune
- 📋 ADR-012 yayında, RESEARCH_BRIEFS/rag-migration.md adopt durumuna geçti
- 📋 Production deploy adımları: (1) `docker compose pull && up -d` (pgvector image swap), (2) migrate auto-run (Program.cs background), (3) `docker exec aykutonpc-web dotnet AykutOnPC.Web.dll --backfill-embeddings`, (4) `--eval-rag` smoke
- 📋 Rollback: `UseRagRetrieval=false` env var → restart. Migration revert (`Down`) ekstra ama şu an opsiyonel

---

## ADR-011 — Anonim Visitor ID: localStorage UUID + First-Party Cookie

- **Tarih:** 2026-05-09
- **Durum:** Kabul edildi
- **Karar verenler:** Senior Dev #1, AppSec, SecOps, PO (Aykut)

**Bağlam:**
INC-001 sonrası VI tanısında (ADR-010, 2026-05-08) test edildi: aynı kullanıcı mobile data + WiFi'den siteye girince 3 farklı IP → `HashedIp` 3 farklı hash → dashboard "3 unique visitor" gösterdi. Pratikte 1 kişi. Ek olarak `HashedIp` günlük rotating salt ile değişiyor — gece 00:00 UTC'de aynı kişi de "yeni" sayılıyor.

Çözmek için kullanıcıya stable bir anonim kimlik gerek. MAC adresi, fingerprinting (canvas/WebGL) tartışıldı ve **reddedildi** — KVKK riski yüksek, "anonim" değil.

**Değerlendirilen Seçenekler:**
1. **localStorage UUID + cookie hybrid** (seçildi) — JS `crypto.randomUUID()`, localStorage'a kaydet, cookie'ye yansıt. Server cookie'yi her request'te alır, page navigation'lar dahil. KVKK'da "anonim agregat sayım için first-party cookie" yorumu güçlü.
2. localStorage + sadece XHR header — page navigation'ları kaçırır (browser navigation header'a custom field eklemez). İlk page load + organic traffic miss.
3. Server-side session cookie — auth-grade, KVKK riski yüksek, fazla kapsam.
4. Browser fingerprinting (canvas hash) — gerçek anti-pattern. KVKK + ePrivacy ihlali, GA bile artık caydırıyor.

**Karar:**
Seçenek 1 — `localStorage` UUID + first-party cookie (`AykutOnPC.VisitorId`, Max-Age 2 yıl, SameSite=Lax). Server `VisitorTrackingMiddleware`'da cookie veya `X-Visitor-Id` header'dan okur. `PageView.VisitorId Guid?` kolonu, `IX_PageViews_VisitorId` indeksi. Dashboard unique sorgusu `COUNT(DISTINCT COALESCE(VisitorId::text, HashedIp))`.

**Rationale:**
- **Stable across networks:** Aynı browser/device farklı IP'lerde aynı UUID taşır → "1 kişi 1 unique"
- **Stable across days:** Cookie + localStorage 2 yıl; HashedIp salt rotation'ından bağımsız
- **KVKK-uyumlu:** Anonim UUID, no PII, geri çevrilemez (UUID → kişi mapping yok). Kullanıcı `localStorage.clear()` veya cookie sil ile resetleyebilir → opt-out var
- **Cookie banner gerekmez:** First-party, anonim, kişiselleştirme yok. AB ePrivacy "strictly necessary" tanımı dışında ama "agregat istatistik için first-party" istisnasına tabi
- **Backwards-compat:** Eski PageView'larda `VisitorId NULL`, `COALESCE` HashedIp'e düşer → eski veri okunabilir
- **Fallback:** JS-disabled / private mode kullanıcılarda VisitorId null kalır, HashedIp eski şekilde çalışır

**Sonuçlar / Trade-off'lar:**
- ✅ Multi-device aynı kişi: Mobile + Desktop = **2 unique** (farklı UUID, doğru — ayrı browser context)
- ✅ Mobile data + WiFi aynı device = **1 unique** (aynı UUID)
- ⚠️ Browser private mode: her oturumda yeni UUID → analytics noise (kabul edilebilir, hangi tracker olursa olsun aynı problem)
- ⚠️ Browser/cihaz değişimi = yeni "kullanıcı" (kabul edilebilir, 1:1 cihaz tracking yapmıyoruz)
- ⚠️ İlk sayfa yüklemesi cookie henüz round-trip etmediği için VisitorId yok → o kayıt HashedIp ile düşer (sonraki tüm sayfalarda VisitorId var)
- 📋 ADR-011, T-#4-002 commit'iyle birlikte yayınlandı
